...
export default (type, object) => {
  ...
  const form =
    type === 'todoForm' ? createTodoForm(object) : createProjectForm(object)
...
function createTodoForm(todo) {
  ...
  submitInput.addEventListener('click', () => {
    todo
      ? editTodo(todo, titleInput.value, descInput.value, dateInput.value)
...
function createProjectForm(object) {
  ...
  submitInput.addEventListener('click', () => {
    object
      ? editProject(object, titleInput.value, descInput.value)
...
function addProject(title, description) {
  ...
  const project = new Project(
    existingProjects.projects.slice(-1)[0]?.id + 1 || 1,
    title,
    description,
  )
